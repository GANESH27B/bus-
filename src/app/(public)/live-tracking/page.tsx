...
import { Clock, MapPin, Search, LocateFixed } from 'lucide-react';
...
export default function LiveTrackingPage() {
  ...
  const EtasList = ({ busesToShow }: { busesToShow: Bus[] }) => (
    <div className="space-y-4">
      {routes
        .filter(r => selectedRoute === 'all' || selectedRoute === r.id)
        .map(route => {
          const activeBusesOnRoute = busesToShow.filter(
            b => b.routeId === route.id
          );
          if (activeBusesOnRoute.length === 0 && selectedRoute !== 'all')
            return null;
          ...
          return (
            <Accordion
              ...
              collapsible
              defaultValue={
                selectedRoute !== 'all' ? `route-${route.id}` : undefined
              }
            >
      ...
      {busesToShow.length === 0 && (
        <p className="text-muted-foreground text-center py-8">
          No active buses found for your search.
        </p>
      )}
    </div>
  );
    ...
    <div className="container mx-auto py-8 px-4 md:px-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Watch your bus move in real-time or switch to a list view.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="lg:col-span-1 h-fit sticky top-20">
          <CardHeader>
            <CardTitle>Find Your Bus or Stop</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <Button onClick={centerOnUser} className="w-full">
